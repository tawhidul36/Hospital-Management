...
def patientclick_view(request):
    if request.user.is_authenticated and is_patient(request.user):
        account_approved = models.Patient.objects.filter(
...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        if accountapproval:
            return redirect('patient-dashboard')
        return render(request, 'hospital/patient_wait_for_approval.html')
    # Default case for unauthenticated or unrecognized users
    return redirect('home_view')  # Make sure 'home' is defined in your URLs
